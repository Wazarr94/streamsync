...
class TestServe:
    ...
    def test_session_verifier_header(self) -> None:
        ...
        with fastapi.testclient.TestClient(asgi_app) as client:
            res = client.post(
                "/api/init",
                json={"proposedSessionId": None},
                headers={"Content-Type": "application/json", "X-Fail": "yes"},
            )
    ...
    def test_session_verifier_cookies(self) -> None:
        ...
        with fastapi.testclient.TestClient(
            asgi_app, cookies={"fail_cookie": "yes"}
        ) as client:
            res = client.post(
                "/api/init",
                json={"proposedSessionId": None},
    ...
    def test_session_verifier_pass(self) -> None:
        asgi_app: fastapi.FastAPI = streamsync.serve.get_asgi_app("./testapp", "run")
        with fastapi.testclient.TestClient(
            asgi_app, cookies={"another_cookie": "yes"}
        ) as client:
            res = client.post(
                "/api/init",
                json={"proposedSessionId": None},
                headers={"Content-Type": "application/json", "X-Success": "yes"},
            )
            assert res.status_code == 200
