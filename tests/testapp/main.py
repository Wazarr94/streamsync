...
def _update_role_chart(state):
    ...
    fig.update_layout(margin=dict(l=20, r=20, t=20, b=50), showlegend=False)
...
def _update_scatter_chart(state):
    main_df = state["main_df"]
    average_role_data = (
        main_df.groupby("role")
        .agg({"length_cm": "mean", "weight_g": "mean"})
        .reset_index()
    )
    fig = px.scatter(
        average_role_data,
...
initial_state = ss.init_state(
    {
        ...
        "articles": {
            "Banana": {"type": "fruit", "colour": "yellow"},
            "Lettuce": {"type": "vegetable", "colour": "green"},
            "Spinach": {"type": "vegetable", "colour": "green"},
        },
        "order_list": [],
