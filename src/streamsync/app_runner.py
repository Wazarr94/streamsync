...
class AppProcess(multiprocessing.Process):
    ...
    def _get_user_functions(self) -> List[str]:
        ...
        all_user_functions = map(
            lambda x: x[0], inspect.getmembers(streamsyncuserapp, inspect.isfunction)
        )
    ...
    def _handle_event(
        self, session: StreamsyncSession, event: StreamsyncEvent
    ) -> EventResponsePayload:
        import traceback as tb
        ...
        res_payload = EventResponsePayload(
            result=result, mutations=mutations, mail=mail
        )
    ...
    def _handle_message(
        ...
        if type == "checkSession":
            return AppProcessServerResponse(
                status="ok", status_message=None, payload=None
            )
...
class AppRunner:
    ...
    def _set_observer(self):
        ...
        self.observer.schedule(
            FileEventHandler(self.reload_code_from_saved),
            path=self.app_path,
            recursive=True,
        )
        self.observer.start()
    ...
    async def update_components(
        ...
        file_contents = {
            "metadata": {"streamsync_version": VERSION},
    ...
    async def handle_event(
        self, session_id: str, event: StreamsyncEvent
    ) -> AppProcessServerResponse:
        return await self.dispatch_message(
            session_id, EventRequest(type="event", payload=event)
        )
